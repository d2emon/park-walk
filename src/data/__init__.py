from article import Article, Link, Shop, GameItem
...
articles = [
    Article(
        0,
        """… Ты решил съездить в Павловский Парк, заодно убив по дороге репера Шляпогрыза. Сев на поселковую """
        """электричку ты вскоре выходишь на вокзале. Парк прямо впереди, ты покупаешь билет и входишь.""",
        (Link("Скоро твое путешествие начнется.", 1), ),
    ),

    Article(
        1,
        """Ты начинаешь игру в Павловском Парке. У тебя нет ничего, кроме пятидесяти рублей. Войдя в парк с юга, ты """
        """двигаешься вперед по длинной алее.""",
        (Link("Иди на (6)", 6), ),
        action=action1
    ),
    Article(2, """2"""),
    Article(
        ...
        (
            Link("Если прямо, туда где ты замечаешь какое – то строение, похожие на магазин, иди на (76)", 76),
            Link("Если свернешь налево, туда, где виднеется какое – то поселение, то иди на (46)", 46),
            Link("Если же направо, где ошивается какой – то мужик, то на (15)", 15),
        ),
    ),
    Article(4, """4"""),
    Article(5, """5"""),
    Article(
        ...
        """Сверившись с картой ты понимаешь, что этот дворец расположен в самой северной части парка. Пройти туда """
        """можно разными путями, и ты знаешь, что некоторые из них на карте не обозначены.\n"""
        """Как бы то ни было, алея вскоре заканчивается ты подходишь к парку. Недалеко стоит ларек с семечками.""",
        (
            Link("Если хочешь зайти, иди на (18)", 18),
            Link("если пройдешь мимо – (3)", 3),
        ),
    ),
    Article(7, """7"""),
    Article(8, """8"""),
    Article(
        9,
        """Через несколько минут ты выходишь на очередную развилку. Ты видишь раздолбанный сожженный магаз, в нем """
        """ничего нет. За него уходит еле заметная тропинка на север, тогда как на восток ведет хорошая ровная """
        """дорога.""",
        (
            Link("На восток\tИди на (22)", 22),
            Link("На север\tИди на (85)", 85),
        ),
    ),
    Article(10, """10"""),
    ...
    Article(
        ...
        """К тебе подходит Состоятельный Мужик в пиджаке, брюках, галстуке и с короткими волосами и спрашивает, не """
        """поможешь ли ты ему в одном деле?""",
        (
            Link("Если хочешь помочь мужику, иди на 68", 68),
            Link("если откажешься и пойдешь мимо, иди на (9)", 9),
        ),
    ),
    Article(16, """16"""),
    Article(17, """17"""),
    Article(
        ...
        (Link("Теперь иди на (3).", 3), ),
        shop=Shop(
            GameItem("СЭМЕЧКИ", 5),
            GameItem("ОРЕХИ ДЛЯ БЕЛОК", 10),
            GameItem("ГРЕЦКИЕ ОРЕХИ", 25),
            GameItem("НОЖ ДЛЯ МЯСА(урон 0.5)", 50, damage=.5),
        ),
    ),
    Article(10, """19"""),
    Article(10, """20"""),
